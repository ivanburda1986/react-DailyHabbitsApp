...
class TodayHabits extends Component {
  ...
  habitCompletionClickHandler = (habitId) => {
    ...
    let habitToUpdate = updatedTodayHabits.filter((habit) => {
    ...
    updatedTodayHabits = updatedTodayHabits.filter((habit) => {
      return habit.id !== habitId;
    });
  ...
  render() {
    //Today habits
    let todayHabits = this.state.todayHabits.map((habit) => <TodayHabit key={habit.id} icon={habit.icon} title={habit.title} subtitle={habit.subtitle} streak={habit.streak} completed={this.completedToday(habit.completed)} clicked={() => this.habitCompletionClickHandler(habit.id)} />);
    //No habits yet
    ...
    return <div className={classes.TodayHabits}>{todayHabits}</div>;
  }
}
